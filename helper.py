...
def fetch_stats(seleceted_user,df):
    if seleceted_user != 'Overall Member':
        df= df[df['user'] == seleceted_user]
...
def emoji_helper(selected_user, df):
    ...
    emojis = []
    for message in df['message']:
        emojis.extend([i for i in message if i in emoji.UNICODE_EMOJI['en']])
...
def day_wise_activity_map(selected_user, df):
    if selected_user != 'Overall Member':
        df = df[df['user'] == selected_user]
    return df['day_name'].value_counts()

def month_wise_activity_map(selected_user, df):
    if selected_user != 'Overall Member':
        df = df[df['user'] == selected_user]
